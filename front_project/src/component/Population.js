...
const PopulationBar = styled.div`
...
const PopulationRank = () => {
  ...
  const rowRenderer = useMemo(() => {
    return (index) => {
      ...
      const widthPercent =
        (country.population /
          Math.max(...allCountries.map((c) => c.population))) *
        100;
      ...
      return (
        <StyledTr key={country.name}>
          ...
          <StyledTd>
            ...
            />
            <Link
              to={`/population/description/${country.englishName.toLowerCase()}`}
          ...
          <StyledTd style={{ position: "relative" }}>
            <PopulationBar style={{ width: `${widthPercent}%` }} />
...
export default React.memo(PopulationRank);
